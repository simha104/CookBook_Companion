...
class Cocktail_Card_Container extends Component {
  ...
  CheckFilters(temp) {
    ...
    if (this.props.filter) {
      ...
      console.log(this.props.filter);
      if (
        temp.Ethnicity != this.props.filter.Ethnicity &&
        this.props.filter.Ethnicity != "All"
      ) {
        console.log("false");
        return false;
      }
      if (
        temp.Difficulty != this.props.filter.Difficulty &&
        this.props.filter.Difficulty != "All"
      ) {
        console.log("false");
        return false;
      }
      if (
        temp.Time != this.props.filter.Time &&
        this.props.filter.Time != "All"
      ) {
        console.log("false");
        return false;
      }
      ...
      <div>
        {this.state.print_food.map(food_list => (
          <Cocktail_Card
            Name={food_list.Name}
            Site={food_list.Site}
            Difficulty={food_list.Difficulty}
            Ethnicity={food_list.Ethnicity}
            Time={food_list.Time}
            Image={"/assets/" + food_list.Image}
            Site={food_list.Site}
          ></Cocktail_Card>
        ))}
      </div>
